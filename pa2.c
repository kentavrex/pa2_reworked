...
Process initialize_child(local_id i, int num_processes, int *balances, Pipe **pipes) {
...
void log_process_start_work(local_id i, FILE *log_events, Process *child_proc) {
    log_process_start(i, log_events, child_proc);
}

void check_started_messages(Process *child_proc, local_id i) {
...
void log_received_all_started(local_id i, FILE *log_events) {
...
void close_outgoing_pipes(Process *child_proc, FILE *log_pipes) {
...
void handle_child_process(local_id i, int num_processes, int *balances, Pipe **pipes, FILE *log_pipes, FILE *log_events) {
    ...
    log_process_start_work(i, log_events, &child_proc);

    check_started_messages(&child_proc, i);
    log_received_all_started(i, log_events);

    perform_bank_operations(&child_proc, log_events);
    close_outgoing_pipes(&child_proc, log_pipes);
}
